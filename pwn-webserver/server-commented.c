        ...
        if ((dir = fdopendir(fd)) != NULL) {
            while ((ent = readdir(dir)) != NULL) {
                if (files_len + 1 > files_cap) {
                    ...
                    files = realloc(files, sizeof(char*) * new_cap); // TODO: use reallocarray instead
                    files_cap = new_cap;
                }
...
int main(int argc, char **argv) {
    ...
    {
        char *query_line = gets(buf); // TODO: don't use gets
    ...
    for (;;) {
        char *header_line = gets(buf); // TODO: don't use gets
        ...
        if (header_count + 1 > header_cap) {
            ...
            headers = realloc(headers, sizeof(struct Header) * new_cap); // TODO: use reallocarray instead
            header_cap = new_cap;
        }
        struct Header h;
        h.name = name;
